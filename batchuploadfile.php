...
if($_POST['action']=="submitbatchfile"){
	...
	$jsonPost= '';
	foreach($_POST['acknowledgmentchecksingle'] as $acknumber){
				$jsonPost.= '{ 
				"AcknowledgementNumber" : "'.$acknumber.'"
				},';
	}
	 
	$jsonPost = rtrim($jsonPost,',');		
	
	$jsonpostnew = '{ "ListOfACKO":['.$jsonPost.']}';
	 
	 if($ProductType=='TAN'){
	 	$url = $serverurlapi."BatchGeneration/RebatchAPIForTAN.php";
	 }else{
	 	$url = $serverurlapi."BatchGeneration/RebatchAPIForPAN.php";
	 } 
	
	
	logger("URL HIT FOR REBATCHING : ". $url); 
	logger("Data post to rebatch api: ". $jsonpostnew); 
	$ch = curl_init();
	curl_setopt($ch, CURLOPT_URL,$url);
	curl_setopt($ch, CURLOPT_POST,1);
	curl_setopt($ch, CURLOPT_POSTFIELDS, $jsonpostnew);
	...
	logger("  Response return from rebatch API: ". $response);
              ...
              <div class="row">
                ...
                <div class="">
				 ...
				 <div class="" style="margin-left: 10px;">
                	<h4 style="color:#99CC33;"><?php if($message!=''){ echo $message; } ?></h4>
				</div>
              </div>
            </div>
          </form>
              ...
              success: function(response){
				...
				console.log(list);
				 ...
				 for(var index = 0; index < list.length; index++) {
				 	src = list[index];
					trHTML +=
						'<tr><td><input type="checkbox" style=" opacity:1; cursor: pointer; height: 17px; width: 50px; margin-top: 3px;" value="'+src.AcknowledgementNumber+'" name="acknowledgmentchecksingle[]" class="deleteack" /></td><td>'+Number(Number(index)+Number(1))+'</td><td class="deta">'
						+ src.AcknowledgementNumber
						+ '</td><td><input type="hidden" name="ProductType" value="'+src.ProductType+'">'
						+ src.ProductType
						+ '</td><td>'
						+ src.BatchNo
						+ '</td></tr>';
