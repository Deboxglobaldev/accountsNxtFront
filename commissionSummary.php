...
$url = $serverurlapi."General/commissionSummaryReportAPI.php";
logger($InfoMessage." URL for API - ".$url); 
...
<div class="hk-wrapper hk-vertical-nav hk-nav-toggle">
  ...
  <div class="hk-pg-wrapper"  style="background-image: url(../html/dist/img/Religare-Dashboard-BG.JPG);">
    ...
    <form action="" method="POST" autocomplete="nope" id="exportfrm" />
      ...
      ?>
            <option value="<?php echo $resultList['Id']; ?>"  <?php if($_POST['commissionPeriod'] == $resultList['Id']){ echo "selected"; } ?> ><?php echo date('d/m/Y',strtotime($resultList['FromDate']))." - ".date('d/m/Y',strtotime($resultList['ToDate'])); ?></option>
      <?php } } } ?>
            </select>
...
<th>Total</th>
  ...
  <th>Generated&nbsp;Date&nbsp;Time</th>
    ...
    ?>
              ...
              <td><?php echo $no; ?></td>
              <td>
                <?php if($resultList->SubLedgerCode != ""){ ?>
                <a href="commissionDetail.php?cid=<?php echo encode($resultList->SubLedgerCode); ?>&bpi=<?php echo encode($resultList->Id); ?>" target="_blank"><?php echo $resultList->BranchCode; ?></a>
