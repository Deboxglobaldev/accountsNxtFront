        ...
        <section class="hk-sec-wrapper contas1">
          <div class="dostas">
            <div style="display: flex;">
              ...
              <h3><?php if($_GET['type']=='batchfile'){ echo 'NSDL Single Batch File Upload [PAN]'; } if($_GET['type']=='accecptedfile'){ echo 'NSDL Accecpted File Upload [PAN]'; } if($_GET['type']=='allotmentfile'){ echo 'NSDL Allotment File Upload [PAN]'; } if($_GET['type']=='batchstatusfile'){ echo 'Batch Status [PAN]'; } if($_GET['type']=='pdf1afile'){ echo 'PDF/1A-A Response Upload'; } ?></h3>
...
$(document).ready(function(){
	$("#misfileupload").click(function(){
        ...
        if(files.length > 0 ){
		   ...
		   $.ajax({
              url: '<?php if($_GET['type']=="batchfile"){ ?>nsdlstageuploadfile.php<?php } if($_GET['type']=="accecptedfile"){ ?>nsdlaccecpteduploadfile.php<?php } if($_GET['type']=="allotmentfile"){ ?>nsdlpanallotuploadfile.php<?php } if($_GET['type']=="batchstatusfile"){ ?>nsdlbatchstatusuploadfile.php <?php } if($_GET['type']=="pdf1afile"){ ?>nsdlpdf1aresponse.php<?php } ?>',
              type: 'post',
              data: formData,
              contentType: false,
              processData: false,
...
<div style="position: fixed; left: 0px; top: 0px; width: 100%; height: 100%; background-color: #000000c7; z-index: 9999; display:none;" id="blkbox">
 ...
 <div style="padding:20px; background-color:#FFFFFF; margin:auto; width:300px; margin-top:10%; text-align:center; border-radius: 10px;color: green;"><img src="img/Spin2.gif" width="100px;"><br>Updating... Please wait it may take few minutes</div>
 </div>
</body>
</html>
<style>
