        ...
        <section class="hk-sec-wrapper contas1">
          <div class="dostas">
            <div style="display: flex;">
              ...
              <h3><?php if($_GET['type']=='batchfile'){ echo 'NSDL Single Batch File Upload [TAN]'; } if($_GET['type']=='accecptedfile'){ echo 'NSDL Accecpted File Upload [TAN]'; } if($_GET['type']=='allotmentfile'){ echo 'NSDL Allotment File Upload [TAN]'; } if($_GET['type']=='batchstatusfile'){ echo 'Batch Status [TAN]'; } if($_GET['type']=='pdf1afile'){ echo 'PDF/1A-A Response Upload'; } ?></h3>
		  ...
		  <div class="container bultable">
            <table class="table">
              <thead>
                <tr class="headline">
                    <th>Sr#</th>
					<th><?php if($_GET['type']=='batchfile'){ echo 'Acknowledegment Number'; } if($_GET['type']=='accecptedfile'){ echo 'Acknowledegment Number'; } if($_GET['type']=='allotmentfile'){ echo 'Acknowledegment Number'; } if($_GET['type']=='batchstatusfile'){ echo 'Batch number'; } ?></th>
					<th>Status</th>
					</tr>
              </thead>
              <tbody id="returndata">
...
$(document).ready(function(){
    ...
    $("#misfileupload").click(function(){
        ...
        if(files.length > 0 ){
           ...
           $.ajax({
              url: '<?php if($_GET['type']=="batchfile"){ ?>nsdlstageuploadfiletan.php<?php } if($_GET['type']=="accecptedfile"){ ?>nsdlaccecpteduploadfiletan.php<?php } if($_GET['type']=="allotmentfile"){ ?>nsdlpanallotuploadfiletan.php<?php }  if($_GET['type']=="batchstatusfile"){ ?>nsdlbatchstatusuploadfiletan.php <?php } if($_GET['type']=="pdf1afile"){ ?>nsdlpdf1aresponsetan.php<?php } ?>',
              type: 'post',
              data: formData,
              contentType: false,
              processData: false,
