        ...
        <section class="hk-sec-wrapper contas1">
          <div class="dostas">
            <div style="display: flex;">
              ...
              <h3>File Submit TAN [<?php echo $_GET['aid']; ?>]</h3>
              <p></p>
...
<?php
$pdfPath = $serverurlapi.'BulkUpload/uploads/tanpdf/'.$_GET['aid'].'.pdf';
$imagePath = $serverurlapi.'BulkUpload/uploads/tanimages/'.$_GET['aid'].'.jpg';

$stageurl = 'dataentrytan.php?aid='.$_GET['aid'].'&formType='.$_GET['formType'];
 
if(trim($subcatofdeductor)!='' || strpos($stage, 'QCF')){
?>
...
$(document).ready(function () {
  $("form").submit(function (event) {
   ...
   for (var index = 0; index < totalfiles; index++) {
	  ...
	  // AJAX request
	   $.ajax({
		 url: 'uploadcurl.php?ProductType=TAN', 
		 type: 'post',
		 data: form_data,
		 //dataType: 'json',
		 contentType: false,
		 ...
		 var src = arr[0];
			...
			var catofdeductor = '<?php echo trim($catofdeductor); ?>';
			  
			  window.location.href = '<?php echo 'dataentrytan.php?aid='.$_GET['aid']; ?>&formType=<?php echo $_GET['formType']; ?>'; 
			}else{
			  $('#status').css({
				 'font-weight' : '600',
				 'color' : 'red'
